...
/**
 * View presenting the list of all records
 * 
 * @author Maciek Makowski
 * @version $Revision: 1.5 $ $Date: 2005/01/23 20:53:34 $
 */
public class RecordsView extends View {
	 ...
	 */
	public RecordsView(MainWindow mainWindow) {
		super(mainWindow);
	}
	...
	public ViewPane createPane(Composite parent) throws MedleyException {
		pane = new RecordListPane(parent, SWT.NONE);
		return pane;
	}
			...
			if (s != null && s.equals(MainWindow.MENU_EDIT)) {
		        ...
		        item.addListener(SWT.Selection, new Listener () {
		            public void handleEvent(Event e) {
		                try {
		                    TagsWindow tagsWin = new TagsWindow(mainWindow.getShell(), Taggable.RECORD);
		                    tagsWin.show();
		        ...
		        item.addListener(SWT.Selection, new Listener () {
		            public void handleEvent(Event e) {
		                try {
		                    RatingsWindow ratingsWin = new RatingsWindow(mainWindow.getShell(), Ratable.RECORD);
		                    ratingsWin.show();
		                } catch (MedleyException ex) {
		                	(new ExceptionWindow(Display.getCurrent(), ex)).show();
		                }
